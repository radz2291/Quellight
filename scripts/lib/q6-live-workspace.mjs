 ...
 *     harness calls it BEFORE any provider turn.
...
export const createQ6LiveWorkspace = ({ removeFn } = {}) => {
  ...
  const workspace = {
     ...
     * turn so an identity mismatch can never reach the provider.
     *
     ...
     * means every persisted byte is credential-free. A missing root
     * scans as empty (nothing was written). The value itself is never
     * emitted.
     *
