...
function allocate(): { workspace: WorkspaceHandle; fixture: { path: string; text: string; byteLength: number; sha256: string } } {
...
const matrixOptionsFor = (workspace: WorkspaceHandle, fixture: { text: string; byteLength: number; sha256: string }) => ({
...
describe('Q6 offline worker-path proof (frozen contract §7)', () => {
  it('the REAL worker process traverses the ENTIRE matrix offline and the parent-side scans and cleanup follow', { timeout: 300_000 }, async () => {
    ...
    const workerPath = fileURLToPath(new URL('../scripts/lib/q6-live-worker.mjs', import.meta.url));
    const spawned = spawnSync(
      process.execPath,
      ['--import', 'tsx', workerPath],
      { cwd: process.cwd(), env: childEnv, encoding: 'utf8', timeout: 240_000 },
    );
    ...
    const byId = (id: string) => result.turns.find((turn: { id: string }) => turn.id.startsWith(id));
  ...
  it('the full matrix traverses in-process too (deterministic re-proof of the same path)', { timeout: 300_000 }, async () => {
    const { workspace, fixture } = allocate();
  ...
  it('an ASYNCHRONOUS evidence failure is awaited, recorded before serialization, and the matrix still completes', { timeout: 300_000 }, async () => {
    const { workspace, fixture } = allocate();
    ...
    const options = {
      ...
      requireCompositionDataDir: (composition: {
        stores: { invocations: { listInvocationsForTurn: (turnId: string) => Promise<unknown> } };
      }, label: string) => {
        workspace.requireCompositionDataDir(composition, label);
